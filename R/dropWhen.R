...
#' @return Returns a column filtered data frame.  Note, does not take into account numeric, integer or character value differences (it will treat them all the same).
#' @examples
...
dropWhen <- function(df, unique = FALSE, trash=NULL, requireAll = TRUE, except = NULL) {
  ...
  fullframe <- merge(filtered, extraColumns)
